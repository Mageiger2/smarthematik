...
const getAngleData = (V, P1, P2, radius) => {
    ...
    // Anker für Leader-Linie: MITTE des Bogen-Bereichs (nicht auf der Bogenlinie),
    // also auf der Bisektor-Richtung bei der Hälfte des Radius.
    const arcMid = { x: V.x + Math.cos(bisector) * (radius * 0.5), y: V.y + Math.sin(bisector) * (radius * 0.5) };
    // Label sitzt AUSSERHALB des Dreiecks auf der Verlängerung der Bisektor-Linie.
    const labelDist = radius * 2.4;
...
const buildSvgBase = (tri, fillCol, mode = "standard") => {
    ...
    if (mode === "standard") {
        ...
        els = [
            ...
            { type: 'rightAngle', v: C, p1: A, p2: B, r },
            { type: 'rightAngle', v: D, p1: C, p2: B, r }
        ];
    }
    return els;
};
...
const generateLevel1 = (lastType) => {
    let type;
    do { type = getRandomInt(1, 6); } while (type === lastType);
...
const generateLevel2 = (lastType) => {
    let type;
    do { type = getRandomInt(1, 3); } while (type === lastType);
...
const msaTasks = [
    {
        ...
        elements: (() => {
            ...
            return [
                ...
                { type: 'rightAngle', v: { x: q, y: 0 }, p1: { x: q, y: h }, p2: { x: c, y: 0 }, r },
    ...
    },
    {
        ...
        elements: [
            ...
            { type: 'rightAngle', v: { x: 8, y: 0 }, p1: { x: 8, y: 5.66 }, p2: { x: 12, y: 0 }, r: 0.7 },
    ...
    },
    {
        ...
        elements: (() => {
            ...
            return [
                ...
                { type: 'rightAngle', v: { x: q, y: 0 }, p1: { x: q, y: hH }, p2: { x: c, y: 0 }, r: 0.3 },
    ...
    },
    {
        ...
        elements: [
            ...
            { type: 'rightAngle', v: { x: 20, y: 0 }, p1: { x: 20, y: 10 }, p2: { x: 25, y: 0 }, r: 2 },
    ...
    },
    {
        ...
        elements: [
            ...
            { type: 'rightAngle', v: { x: 6.4, y: 0 }, p1: { x: 6.4, y: 4.8 }, p2: { x: 10, y: 0 }, r: 0.8 },
...
const TrigonometrieTrainer = () => {
    ...
    const loadTaskForDiff = (diff) => {
        ...
        if (diff === 'leicht') {
            ...
            historyRef.current.l1 = newTask.typeId;
        } else if (diff === 'mittel') {
            newTask = generateLevel2(historyRef.current.l2);
            historyRef.current.l2 = newTask.typeId;
        } else if (diff === 'schwer') {
            newTask = generateLevel3(historyRef.current.l3);
            historyRef.current.l3 = newTask.typeId;
        } else { // pruefung
            let pool = examPool;
    ...
    const renderCompletedStep = (step) => {
        ...
        if (step.type === 'formula_selection') {
            content = <span className="text-orange-800">{answer}</span>;
            ...
            content = <span className="text-orange-800" style={{ fontFamily: "'Cambria Math', 'Times New Roman', serif", fontStyle: 'italic' }}>{text}</span>;
        } else if (step.type === 'calculation') {
            const text = step.template.replace('[input]', fmtT(answer));
            content = <span className="text-green-800" style={{ fontFamily: "'Cambria Math', 'Times New Roman', serif", fontStyle: 'italic' }}>{text}</span>;
        }
        return (
            <div key={step.id} className="bg-white p-4 rounded-xl shadow-sm border-l-4 border-green-300 mb-3 opacity-90">
                <div className="flex items-center text-slate-500 font-bold mb-1 text-xs uppercase tracking-wide">
                    <CheckCircle className="w-4 h-4 mr-1 text-green-500" /> Schritt erledigt
                </div>
                <p className="text-sm text-slate-600 mb-2">{step.goal}</p>
                <div className="bg-slate-50 px-3 py-1.5 rounded border border-slate-200 text-lg font-bold inline-block">
                    {content}
                </div>
            </div>
        );
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                        ...
                        <div className="lg:col-span-2">
                            ...
                            <div className="flex flex-col">
                                {currentTask.steps.slice(0, currentStepIndex).map(s => renderCompletedStep(s))}
                            </div>
